...
With island, you define your data using Plain Old Algebraic Datatypes, like this.
    ...
    makeStructuredProduct ''Gradient

`makeStructuredProduct` uses Template Haskell to generate the following instances.

    instance Structured RGB where
      type Shape RGB f branch leaf = Rec f (FieldTypes branch leaf RGB)

      _Shape :: Iso' RGB (Shape RGB Identity Identity Identity)
      _Shape = _Rec

      data Piece RGB branch leaf a where
        Red   :: Piece RGB (leaf Word8)
        Green :: Piece RGB (leaf Word8)
        Blue  :: Piece RGB (leaf Word8)

      pieceTraversal :: Piece RGB t t a
                     -> Traversal (Shape RGB f t t) (Shape RGB g t t)
                                  (f (t a))         (g (t a))
      pieceTraversal = pieceLens

    instance StructuredProduct RGB where
      type FieldTypes branch leaf RGB = '[ leaf Word8
                                         , leaf Word8
                                         , leaf Word8
                                         ]

      _Rec :: Iso' RGB (Rec Identity (FieldTypes RGB Identity Identity))
      _Rec = ...
    ...
    instance Structured Gradient where
      type Shape Gradient f branch leaf = Rec f (FieldTypes branch leaf Gradient)

      _Shape :: Iso' Gradient (Shape Gradient Identity Identity Identity)
      _Shape = _Rec

      data Piece Gradient branch leaf a where
        StartColor :: Piece Gradient (branch RGB)
        EndColor   :: Piece Gradient (branch RGB)
        IsCircular :: Piece Gradient (leaf   Bool)

      pieceTraversal :: Piece Gradient t t a
                     -> Traversal (Shape Gradient f t t) (Shape Gradient g t t)
                                  (f (t a))              (g (t a))
      pieceTraversal = pieceLens

    instance StructuredProduct Gradient where
      type FieldTypes Gradient branch leaf = '[ branch RGB
                                              , branch RGB
                                              , leaf   Bool
                                              ]

      _Rec :: Iso' Gradient (Rec Identity (FieldTypes Gradient Identity Identity))
      _Rec = ...

      pieceLens :: Piece Gradient t t a
                -> Lens (Rec f (FieldTypes Gradient t t)) (Rec g (FieldTypes Gradient t t))
                        (f (t a))                         (g (t a))
      pieceLens = ...

That's a lot of code, but for now all we're interested in is the `_Rec` isomorphism, which allows us to convert back and forth between our POAD representation and a vinyl `Rec`. Island provides another isomorphism called `_RecTree` which converts back and forth between our POAD representation and island's `RecTree`. This makes is possible to use whichever representation is the most appropriate for the current task: `Rec` when you want to uniformly manipulate the direct children, `RecTree` when you want to uniformly manipulate the leaves, and the friendlier POAD representation for all your other tasks.

`_RecTree` determines which fields are leaves and which ones are nested records using the `FieldTypes` metadata in the generated `StructuredProduct` instance. This information is in turn generated by `makeStructuredProduct` based on whether it can find a `StructuredProduct` instance for each field type. Early calls to `makeStructuredProduct` cannot see the instances generated by later calls to `makeStructuredProduct`, so unlike most top-level declarations in Haskell, the order in which you write your `makeStructuredProduct` calls matters.
